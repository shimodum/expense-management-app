...
3. **引数**: `ExpenseReport $expenseReport`, `array $validated`(create同様の入力項目。領収書画像を差し替える場合は`receipt_image`、既存画像を削除する場合はその旨を示す入力を含む)
...
6. **処理手順**:
   1. 領収書画像の差し替え・削除が指定されていれば、新ファイルの保存または`receipt_image_path`のクリアを行う(8章参照。旧ファイルの削除はDB更新の後に行う)
   2. `expense_reports`の該当行をUPDATEする(expense_date, amount, expense_category_id, payee, description, receipt_image_pathを更新。`status`・`rejection_reason`は変更しない)
   3. 手順1で新ファイルへの差し替え、または削除を行った場合、DB更新の成功を確認したうえで旧ファイルを削除する
   4. 更新後の`ExpenseReport`を返す
7. **更新対象テーブル**: `expense_reports`
...
12. **処理失敗時の扱い**: 新ファイルの保存に失敗した場合は例外を伝播させ、DB更新を行わない。DB更新が失敗した場合は例外を伝播させ、旧ファイルは削除しない(削除は手順3でDB更新成功後にのみ行うため)。旧ファイルの削除に失敗した場合、DBは既に正しい状態のため、孤立ファイルが残るのみで許容する。
...
12. **処理失敗時の扱い**: DB更新が失敗した場合は例外を伝播させ、トランザクションはロールバックされる(通常の`QueryException`等としてLaravel標準の例外処理に委ねる)。
13. **Serviceでは行わない処理**: 認可判定、`comment`の形式検証(文字数上限等はFormRequestの責務)、redirect/viewの生成。
...
3. UPDATEの成功を確認したうえで、旧ファイルを削除する
